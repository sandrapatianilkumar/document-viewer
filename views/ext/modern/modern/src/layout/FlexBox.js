...
Ext.define('Ext.layout.FlexBox', {
    ...
    setItemFlex: function(item, flex) {
        var element = item.element,
            ...
            propName = Ext.browser.is.WebKit ? '-webkit-box-flex' :
                Ext.browser.is.IE ? '-ms-flex' :
                'flex';

        element.toggleCls(Ext.baseCSSPrefix + 'flexed', !!flex);

        if (Ext.browser.is.WebKit) {
            style.setProperty(propName, flex ? flex : '', null);
        } else {
            style.setProperty(propName, (flex ? flex + ' 0 0px' : ''), null);
        }
    },
