        ...
        if (target && (match = target.className.match(me.actionIdRe))) {
            ...
            if (item && !disabled) {
                ...
                if (type === 'mousedown' && !me.getView().actionableMode) {
                    ...
                    // 
                    // If the handler moved focus outside of the view, do not allow this event
                    // to propagate to cause any navigation.
                    if (view.destroyed || 
                        (view.el && !view.el.contains(Ext.Element.getActiveElement()))) {
                        return false;
                    }
                }
            }
